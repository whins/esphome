import esphome.codegen as cg
import esphome.config_validation as cv
from esphome.components import binary_sensor
from esphome.const import (
    CONF_ID, DEVICE_CLASS_OCCUPANCY
)
from . import ak9753_ns, AK9753Component, CONF_AK9753_ID

DEPENDENCIES = ["i2c"]

AK9753PresenceBinarySensor = ak9753_ns.class_(
    "AK9753PresenceBinarySensor", binary_sensor.BinarySensor, cg.Component
)

CONFIG_SCHEMA = binary_sensor.binary_sensor_schema(
    device_class=DEVICE_CLASS_OCCUPANCY,
).extend({
    cv.GenerateID(): cv.declare_id(AK9753PresenceBinarySensor),
    cv.GenerateID(CONF_AK9753_ID): cv.use_id(AK9753Component),
})

async def to_code(config):
    paren = await cg.get_variable(config[CONF_AK9753_ID])
    sens = await binary_sensor.new_binary_sensor(config)
    var = cg.new_Pvariable(config[CONF_ID], sens)
    cg.add(paren.register_binary_sensor(var))